        ...
        public static partial class KittyViewHelper
        {
            ...
            public static string GenerateUpdatePage(Type T, UpdateViewOptions options)
            {
                ...
                if (options.ReferencedByTypes != null)
                {
                    foreach (var ByType in options.ReferencedByTypes)
                    {
                        StringBuilder.AppendLine(
                            KittyViewHelper.GenerateVueButton("New " + ByType.Name, "New" + ByType.Name));
                    }
                }
                ...
                foreach (var field in FieldInfos)
                {
                    ...
                    {
                        StringBuilder.AppendLine(KittyViewHelper.GenerateVueNumberInput(field, true));
                        continue;
                    }
                    ...
                    {
                        case "String":
                            StringBuilder.AppendLine(KittyViewHelper.GenerateVueTextInput(field,options.DisableUpdate));
                            break;
                        case "Int32":
                        case "Int64":
                        case "Double":
                        case "Float":
                            ...
                            StringBuilder.AppendLine(KittyViewHelper.GenerateVueNumberInput(field,options.DisableUpdate));
                            break;
                        case "DateTime":
                            StringBuilder.AppendLine(KittyViewHelper.GenerateVueDateTimeInput(field,options.DisableUpdate));
                            break;
                        default:
                            break;
                    }
                ...
                if (options.ReferencedByTypes != null)
                {
                    foreach (var ByType in options.ReferencedByTypes)
                    {
                        ...
                        string snake = ComponentName.ToUnderscoreCase();
                        StringBuilder.AppendLine($"<{snake} :reference-id=\"DataModel.Id\" ></{snake}>");
...
import {{ {options.RequestObjectName}, {T.Name}, {options.ListOneRequestObjectName} }} from '@/shared/dtos';");
                ...
                {
                    StringBuilder.AppendLine(AdditionalImport);
                }
