...
namespace KittyHelper
{
    public static partial class KittyHelper
    {
        ...
        {
            public static string GenerateSelectAgainstAssociationPage(Type T, SelectAgainstAssociationViewOptions options)
            {
                StringBuilder StringBuilder = new();
                StringBuilder.AppendLine("<template>");
                StringBuilder.AppendLine("<section><div class='container'> ");
                StringBuilder.AppendLine($"<h4>Select {T.Name}s</h4>");
                ...
                StringBuilder.AppendLine(GenerateVueCard(DisplayFieldName, "``", DisplayTextName, GenerateVueButton(
                    "Select",
                    $"SelectAgainstAssociation({KeyField})"), vFor));
...
export default class {options.ComponentName} extends Vue {{");
                ...
                StringBuilder.AppendLine(" @VModel({ type: Number }) SelectAgainstAssociationedId!:  number");
                StringBuilder.AppendLine(
                    $"@Watch('SearchText') changed(old:string,newx :string) {{  this.SelectAgainstAssociation{T.Name}();  }}");
                ...
                StringBuilder.AppendLine($"async SelectAgainstAssociation{T.Name}() {{");
                ...
                StringBuilder.AppendLine("}");
                StringBuilder.AppendLine(
                    $" Previous(){{  this.After =  this.After > 50? this.After - 50:0;   this.SelectAgainstAssociation{T.Name}();   ; }} ");
                StringBuilder.AppendLine($" created(){{  this.SelectAgainstAssociation{T.Name}(); }} ");
                StringBuilder.AppendLine(
                    $" Next(){{  this.After  = this.DataModel[this.DataModel.length -1].{options.DataBaseObjectIdField} +1  ;  this.SelectAgainstAssociation{T.Name}(); }} ");
                StringBuilder.AppendLine(
                    " SelectAgainstAssociation(id:number){  this.SelectAgainstAssociationedId=id;} ");
                StringBuilder.AppendLine("}");
            ...
            public class SelectAgainstAssociationViewOptions
            {
