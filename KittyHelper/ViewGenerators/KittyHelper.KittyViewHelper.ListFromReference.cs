...
export default class {options.ComponentName} extends Vue {{");
                ...
                StringBuilder.AppendLine($"@Watch('SearchText') changed(old:string,newx:string) {{  this.ListFromReference{T.Name}();  }}");
                StringBuilder.AppendLine($"Message : string = \"\"");
                StringBuilder.AppendLine($" @Prop(Number) readonly  reference_id : number ");
                ...
                StringBuilder.AppendLine(
                    $" const Response = await client.{options.HttpVerb.ToLower()}(new {options.RequestObjectName}({{  ReferenceId:this. reference_id,   {options.RequestObjectField} : this.After, SearchText:this.SearchText }} ));");
